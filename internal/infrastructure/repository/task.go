...
func SearchTasksByText(data string, limit int) ([]*dto.Task, error) {
	var rows *sql.Rows
...
func SearchTasksByDate(searchDate time.Time, limit int) ([]*dto.Task, error) {
	var rows *sql.Rows
	...
	rows, err := bootstrap.Db.Query(`
		...
		sql.Named("date", searchDate.Format(datePkg.DateFormat)),
		sql.Named("limit", limit),
	)
